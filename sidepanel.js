	...
	browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
	  var currUrl = tabs[0].url;
		browser.storage.local.get({homeTab:""}, function(items){  
			browser.tabs.sendMessage(items.homeTab, {action : 'getTid', data:{url:currUrl}, opts:"edit"},function(res){
	...
	x.style.display = "none";
		...
		browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
		  var currUrl = tabs[0].url;
			...
			browser.storage.local.get({homeTab:""}, function(items){  
				browser.tabs.sendMessage(items.homeTab, {action : 'putTid', 
					...
					//console.log(res);
					browser.tabs.sendMessage(items.homeTab, {action : 'getTid', data:{url:currUrl},opts:"render"},function(res){
	...
	//first save note
		...
		browser.storage.local.get({homeTab:""}, function(items){  
		  ...
		  current.tags = JSON.stringify(tags);
			if (current.url && editvalue) browser.tabs.sendMessage(items.homeTab, {action : 'putTid', opts:"puts",data:current},function(res){
				//console.log(res);
			});
			browser.tabs.sendMessage(tabs[0].id, {action : 'getMeta'}, function (meta)	{ 
				console.log("meta",meta.description,meta.mediaImage);
...
browser.tabs.onActivated.addListener((actTabInfo) => {
  browser.tabs.get(actTabInfo.tabId, (tab) => {
	  browser.windows.getCurrent({} ,function(Window) {//console.log("onActivated");
...
main();
function main(){
	browser.storage.local.get({conected:"not here"}, function(items){
				document.querySelector('#home').innerHTML = items.conected;
		} 
